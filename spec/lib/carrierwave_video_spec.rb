  ...
  describe '#encode_video' do
    ...
    context 'with no options set' do
      ...
      it 'provides a default for the resolution' do
        expect(movie).to receive(:transcode) do |_path, opts, _codec_opts|
    ...
    context 'with watermark set' do
      ...
      it 'appends watermark params to custom params for ffmpeg' do
        expect(movie).to receive(:transcode) do |path, opts, codec_opts|
          ...
          expect(opts[:video_codec]).to eq('libvpx')
          expect(opts[:audio_codec]).to eq('libvorbis')
          watermark_params = ['-vf', 'movie=path/to/file.png [logo]; [in][logo] overlay=5:main_h-overlay_h-5 [out]']
          expect(opts[:custom]).to eq(%w[-b 1500k -ab 160000 -f webm -g 30] + watermark_params)
      ...
      it 'only requires path watermark parameter' do
        expect(movie).to receive(:transcode) do |path, opts, codec_opts|
          ...
          expect(opts[:video_codec]).to eq('libvpx')
          expect(opts[:audio_codec]).to eq('libvorbis')
          watermark_params = ['-vf', 'movie=path/to/file.png [logo]; [in][logo] overlay= [out]']
          expect(opts[:custom]).to eq(%w[-b 1500k -ab 160000 -f webm -g 30] + watermark_params)
    ...
    context 'with custom passed in' do
      ...
      it 'maintains the watermark params' do
        expect(movie).to receive(:transcode) do |_path, opts, _codec_opts|
          watermark_params = ['-vf', 'movie=path/to/file.png [logo]; [in][logo] overlay= [out]']
          expect(opts[:custom]).to eq(%w[-preset slow] + watermark_params)
        end
    ...
    context 'given a block' do
      ...
      let(:params) do
        {
            ...
            custom: %w[-b 1500k -ab 160000 -f webm -g 30]
        }
      end
      ...
      it 'calls the block, with the movie file and params' do
        expect(movie).to receive(:transcode) do |_path, format_opts, _codec_opts|
        ...
        end.to yield_with_args(movie, params)
      end

      it 'allows the block to modify the params' do
        block = proc { |_input, params| params[:custom] = %w[-preset slow] }
      ...
      it 'evaluates the final params after any modifications' do
        block = proc do |_input, params|
          params[:custom] = %w[-preset slow]
          params[:watermark][:path] = 'customized/path'
        end

        expect(movie).to receive(:transcode) do |_path, format_opts, _codec_opts|
          watermark_params = ['-vf', 'movie=customized/path [logo]; [in][logo] overlay= [out]']
          expect(format_opts[:custom]).to eq(%w[-preset slow] + watermark_params)
        end
